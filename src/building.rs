    ...
    fn building_outside(
        ...
        let inside = {
            ...
            let max_rad = (context.width/2) as f64
                * (((noise.get_value(theta) + 1.0) / 2.0).powi(2) * 0.8+0.2)
                * scaler*1.8;
            rad < max_rad && !(rad >max_rad*0.5 && context.get_random_float()>1.0-(rad/max_rad).powi(4))
        };
        outside || !inside
    }
    prof_frame!("Building::filter_buildings()");
    let mut out = vec![];
    let noise = NoiseGenerator1d::new(TAU ,PI/8.0, 8, context);
    for b in buildings {
